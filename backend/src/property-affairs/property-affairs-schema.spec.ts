...
describe('property-affairs schema', () => {
  ...
  it('exposes normalized visibility scope and viewer relations', () => {
    ...
    expect(schema).toMatch(
      /visibilityScope\s+PropertyAffairVisibilityScope\s+@default\(ALL\)\s+@map\("visibility_scope"\)/,
    );
    expect(schema).toMatch(/viewers\s+PropertyAffairViewer\[\]/);
    expect(schema).toMatch(
      /propertyAffairViewerEntries\s+PropertyAffairViewer\[\]/,
    );
    expect(modelBlock(schema, 'PropertyAffairViewer')).toContain(
      '@@id([affairId, userId])',
    );
    expect(modelBlock(schema, 'PropertyAffairViewer')).toContain(
      '@@index([userId, affairId])',
    );
    const affair = Prisma.dmmf.datamodel.models.find(
      (model) => model.name === 'PropertyAffair',
    );
    const scope = affair?.fields.find(
      (field) => field.name === 'visibilityScope',
    );
    const viewers = affair?.fields.find((field) => field.name === 'viewers');
    const viewer = Prisma.dmmf.datamodel.models.find(
      (model) => model.name === 'PropertyAffairViewer',
    );
    expect(scope).toMatchObject({
      kind: 'enum',
      type: 'PropertyAffairVisibilityScope',
      dbName: 'visibility_scope',
    });
    expect(viewers).toMatchObject({
      kind: 'object',
      type: 'PropertyAffairViewer',
    });
    ...
    const migration = readFileSync(
      join(
        prismaDirectory,
        'migrations',
        '20260914090000_property_affair_visibility',
        'migration.sql',
      ),
      'utf8',
    );
    expect(migration).toMatch(
      /FOREIGN KEY \(`affair_id`\)[\s\S]*ON DELETE CASCADE/,
    );
    expect(migration).toMatch(
      /FOREIGN KEY \(`user_id`\)[\s\S]*ON DELETE CASCADE/,
    );
