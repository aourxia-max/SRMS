...
本次隔离分支验证结果：数据库结构迁移成功且历史收款/押金流水数量不变；后端 57 个测试套件 297 项、前端 25 个测试文件 138 项、后端 E2E 4 个套件 15 项全部通过，前后端构建、Lint 和 Prisma 校验通过。本机测试合同 ID 25 的 10,000.00 元押金自动入账、退租快照、财务余额增量、重复来源拒绝和历史合同不补账均已验证。

## 退租结算退还租金（2026-08-31）

退租结算现支持管理员填写一项“退还租金”，系统自动计算可回冲上限、锁定原租金收款分配，并与应退押金、应退预收款合并为一次实际退款。最终确认会在同一事务中同步更新账单、收款、押金/预收款、结算、合同、房态和安全审计；普通退款、收款作废及并发退租不能重复占用同一金额。

本轮验证结果：后端 84 个测试套件 615 项、前端 40 个测试文件 286 项、真实 MySQL E2E 3 个套件 16 项全部通过；前后端构建、Lint、Prisma schema、27 个 migration 状态和空库差异检查均通过。测试环境更新前的首页与 API 健康检查返回 HTTP 200。

完整业务边界、迁移、备份核验、自动化结果和手工验收步骤见 `docs/checkout-rent-refund-acceptance.md`。
