...
import { propertyAffairAvailabilityLabel, propertyAffairPriorityLabel, propertyAffairRelationStatusLabel, propertyAffairStatusLabel } from '../utils/property-affair-labels'
    ...
    <template v-else-if="affair">
      ...
      <el-card>
        ...
        <div class="relation-groups">
          <section><h3>楼栋</h3><el-empty v-if="!affair.buildings.length" description="未关联楼栋" :image-size="44" /><div v-for="item in affair.buildings" :key="item.id" class="relation-item"><router-link v-if="item.currentStatus" :data-test="`building-link-${item.id}`" :to="{ name: 'properties', query: { buildingId: item.id } }">{{ relationText(item) }}</router-link><span v-else :data-test="`building-missing-${item.id}`">{{ relationText(item) }}</span><small>关联时：{{ item.snapshotLabel }} · 当前状态：{{ propertyAffairRelationStatusLabel('building', item.currentStatus) }} · {{ propertyAffairAvailabilityLabel(item.available) }}</small></div></section>
          <section><h3>房源</h3><el-empty v-if="!affair.rooms.length" description="未关联房源" :image-size="44" /><div v-for="item in affair.rooms" :key="item.id" class="relation-item"><router-link v-if="item.currentStatus" :data-test="`room-link-${item.id}`" :to="{ name: 'room-detail', params: { id: item.id } }">{{ relationText(item) }}</router-link><span v-else :data-test="`room-missing-${item.id}`">{{ relationText(item) }}</span><small>关联时：{{ item.snapshotLabel }} · 当前状态：{{ propertyAffairRelationStatusLabel('room', item.currentStatus) }} · {{ propertyAffairAvailabilityLabel(item.available) }}</small></div></section>
          <section><h3>承租人</h3><el-empty v-if="!affair.tenants.length" description="未关联承租人" :image-size="44" /><div v-for="item in affair.tenants" :key="item.id" class="relation-item"><router-link v-if="item.currentStatus" :data-test="`tenant-link-${item.id}`" :to="`/tenants/${item.id}`">{{ relationText(item) }}</router-link><span v-else :data-test="`tenant-missing-${item.id}`">{{ relationText(item) }}</span><small>关联时：{{ item.snapshotLabel }} · 当前状态：{{ propertyAffairRelationStatusLabel('tenant', item.currentStatus) }} · {{ propertyAffairAvailabilityLabel(item.available) }}</small></div></section>
          <section><h3>合同</h3><el-empty v-if="!affair.contracts.length" description="未关联合同" :image-size="44" /><div v-for="item in affair.contracts" :key="item.id" class="relation-item"><router-link v-if="item.currentStatus" :data-test="`contract-link-${item.id}`" :to="{ name: 'contracts', query: { tab: 'detail', contractId: item.id } }">{{ relationText(item) }}</router-link><span v-else :data-test="`contract-missing-${item.id}`">{{ relationText(item) }}</span><small>关联时：{{ item.snapshotLabel }} · 当前状态：{{ propertyAffairRelationStatusLabel('contract', item.currentStatus) }} · {{ propertyAffairAvailabilityLabel(item.available) }}</small></div></section>
        </div>
      </el-card>
