...
<template>
  <section class="completed-contracts-panel">
    <div class="completed-contracts-panel__title">
      ...
      <form
        class="completed-contracts-panel__search"
    ...
    <div v-if="loading" class="completed-contracts-panel__empty">
      正在加载已退租合同…
    </div>
    <div
      v-else-if="!result.items.length"
      class="completed-contracts-panel__empty"
    >
      暂无符合条件的已退租合同
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="item in result.items" :key="item.settlementId">
            ...
            <td class="completed-contracts-panel__money">
...
<style scoped>
.completed-contracts-panel {
  display: grid;
  gap: 16px;
}
.completed-contracts-panel__title {
  display: flex;
  align-items: flex-end;
  justify-content: space-between;
  gap: 16px;
}
.completed-contracts-panel__title h2 {
  margin: 0 0 6px;
  font-size: 20px;
}
.completed-contracts-panel__title p {
  margin: 0;
  color: #66758b;
}
.completed-contracts-panel__search {
  display: flex;
  gap: 8px;
}
.completed-contracts-panel__search input {
  min-width: 280px;
  min-height: 40px;
...
.completed-contracts-panel__empty,
.completed-contracts-panel__table-wrap {
...
.completed-contracts-panel__money {
  color: #c97a13;
  font-weight: 600;
}
.primary-button,
.link-button,
.completed-contracts-panel__pagination button {
  min-height: 36px;
  padding: 0 14px;
  border-radius: 6px;
  font: inherit;
...
.completed-contracts-panel__pagination {
  display: flex;
  align-items: center;
