...
describe('合同作废动作幂等会话', () => {
  it('页面重新挂载后从 sessionStorage 恢复同一 submission key 且不保存敏感表单内容', () => {
    ...
    const first = createContractVoidActionSession(7, factory, storage)
    expect(first.submissionKey('digest-contract-86-v1')).toBe('submit-session-000001')

    const second = createContractVoidActionSession(7, factory, storage)
  ...
  it('表单提交响应丢失时复用 submission key，只有开始全新表单才轮换', () => {
    ...
    const session = createContractVoidActionSession(7, factory)
  ...
  it('execution key 按申请跨页面恢复且终态后同时清除 execution 与 submission key', () => {
    ...
    const third = createContractVoidActionSession(7, factory, storage)
  ...
  it('同一标签切换用户后不复用前一用户的 key 或待确认定位', () => {
    const storage = memoryStorage()
    const factory = vi.fn().mockReturnValueOnce('user-a-submit').mockReturnValueOnce('user-b-submit')
    const userA = createContractVoidActionSession(7, factory, storage)
    const userB = createContractVoidActionSession(8, factory, storage)

    expect(userA.submissionKey('same-digest')).toBe('user-a-submit')
    expect(userB.hasSubmissionKey('user-a-submit')).toBe(false)
    expect(userB.submissionKey('same-digest')).toBe('user-b-submit')
    expect(userA.hasSubmissionKey('user-b-submit')).toBe(false)
    expect([...Array(storage.length)].map((_, index) => storage.key(index))).toEqual(
      expect.arrayContaining([
        expect.stringContaining('user:7:submit:same-digest'),
        expect.stringContaining('user:8:submit:same-digest'),
      ]),
    )
  })
})
