...
const props = withDefaults(defineProps<{
  ...
  role: ContractRole
  loading?: boolean
}>(), { contract: null, bills: () => [], files: () => [], changes: () => [], loading: false })

const emit = defineEmits<{ back: []; rebate: [] }>()
      ...
      <div class="detail-grid">
        <section class="contract-card detail-main">
          <el-tabs v-model="activeSection">
            ...
            <el-tab-pane label="收款记录" name="payments"><el-empty :image-size="64" description="收款记录请进入收款管理查看" /></el-tab-pane>
            ...
            <el-tab-pane label="附件" name="files">
              <el-table :data="files" empty-text="暂无合同附件"><el-table-column prop="originalName" label="文件名" /><el-table-column prop="mimeType" label="类型" /><el-table-column label="大小"><template #default="{ row }">{{ row.sizeBytes ? `${Math.ceil(row.sizeBytes / 1024)} KB` : '—' }}</template></el-table-column></el-table>
            </el-tab-pane>
