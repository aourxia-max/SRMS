...
- 仅当应退押金、应退预收款、应退租金和待补收余额全部完成处理并归零，才允许退租结算变为`COMPLETED`并结束合同。
- 无押金或押金全部抵扣仅表示押金分项已归零，不得绕过预收款退款、租金退款或待补收款处理。
...
净资金流 = 外部资金流入 - 外部资金流出
租金及押金入账合计 = payment_date位于所选日期范围、status为CONFIRMED或PARTIALLY_REFUNDED、payment_category为RENT、PREPAYMENT或DEPOSIT的收款金额合计
```

退租合并退款内部的押金、预收款和租金拆分只用于分类与追溯，不得重复形成三笔外部资金流。

“租金及押金入账合计”是资金发生日口径的历史入账额，排除作废收款和退租补收；普通退款与退租合并退款继续作为独立资金流出，不倒扣历史入账。该汇总不替代账期净应收，财务中心仅隐藏顶部净应收卡片，明细、导出和未收计算仍保留净应收。
