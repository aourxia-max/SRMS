...
3. 备份状态必须为 `SUCCESS`，数据库文件必须实际存在且可读取，并且重新计算的 SHA-256 必须与备份记录完全一致；文件缺失、不可读或被替换都会在事务前停止。
...
6. `--preflight-fingerprint` 必须精确等于本次人工复核的最新预检输出；执行进入事务后会对合同 ID、关联范围及表计数重新生成指纹，任何变化都会在首次删除前回滚。
7. 预检读取 `INFORMATION_SCHEMA.KEY_COLUMN_USAGE`；发现静态清单之外的新外键时立即停止。
8. 关联数据删除、合同删除和房态重算必须在同一个数据库事务中完成，任一步失败整体回滚。
9. 文件资产只有在所有已知业务引用均解除后才删除；物理文件仅在事务提交后处理，失败项会列入 `manualFileCleanup`。
...
- 数据库当前外键清单；
- 未纳入静态依赖清单的外键；
- 对精确合同 ID、关联数据 ID 和各表计数计算的 64 位 SHA-256 `fingerprint`。

把完整 JSON 输出保存到本次变更记录中。若 `unknownDependencies` 非空，不得继续。人工复核后必须使用该次输出中的 `fingerprint`；范围或计数变化后，旧指纹会被拒绝，必须重新预检和审批。
...
```powershell
npm --prefix backend run cleanup:tiered -- --mode=execute --environment=<test或production> --backup-no=<本次成功备份编号> --preflight-fingerprint=<本次预检输出的fingerprint> --confirmation=DELETE_ALL_TIERED_CONTRACT_HISTORY --final-authorization=OWNER_APPROVED_EXECUTION
```

生产执行前必须再次完成以下人工检查：

1. 向项目负责人展示刚生成的生产预检 JSON，逐项核对合同、关联表计数和 `fingerprint`。
2. 展示备份编号、状态、保留期和校验和，确认对应备份文件存在、可读取且校验一致。
...
- 授权、备份、预检指纹或未知外键检查失败：不启动事务，修正问题后重新预检。
- 事务内范围指纹变化：事务在任何删除前回滚；重新预检、复核并取得新的最终授权。
- 事务内其他失败：数据库自动整体回滚，禁止手工续删；保留错误日志并重新分析依赖。
