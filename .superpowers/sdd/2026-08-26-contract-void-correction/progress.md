...
Task 12 fix: refreshSnapshot 成功读取按角色选择 include；ADMIN 自己刷新时不查询／返回 reversals，SUPER_ADMIN 保持完整。RED 为 ADMIN 刷新结果含 reversals；GREEN 为 contract-void-requests.service.spec.ts 31/31，diff-check 通过。
...
Task 12 fixed-contract confirmation room gate: RED direct/draft unit coverage failed exactly on the missing rooms FOR UPDATE ordering and missing ReadCommitted transaction options (4 failed, 28 passed). GREEN locks the room before locked-state reload, conflict checks and contract writes for both entry paths, with explicit ReadCommitted; focused unit passed 32/32, normal MySQL contract-deposit E2E passed 2/2 after 26/26 migration status, and changed-file Prettier/ESLint plus diff check passed.
Task 12 finance correction: RED confirmed ordinary terminal payments leaked, matching correction terminals were unavailable to the selector, historical nonzero COMMISSION reversals leaked, and commission writer emitted a nonzero financial row. GREEN: matching PAYMENT reversal IDs retain only correction-linked terminal receipts; COMMISSION is excluded from cash-flow query and mapper; writer emits a 0.00 indicator with null balances and originalAmount metadata. Focused finance/writer/export 4 suites / 22 tests passed; changed-files ESLint and diff check passed.
...
Task 12 Ruling: 完整冲销仅超级管理员可查询及显示；任何管理员仅能取消本人 PENDING 申请；同房有效占用统一为 PENDING_START／ACTIVE／PENDING_CHECKOUT，预览和执行器按合同 ID 升序查询。
Task 12 RED/GREEN: RED 暴露 ADMIN 详情加载 reversals、SUPER_ADMIN 可取消他人、预览包含 DRAFT／ENDED、执行器缺稳定排序与前端越权显示／取消；GREEN 为 backend 4 suites／66 tests、frontend panel 30/30，git diff --check 通过；未改金额、数据库或环境文件。
